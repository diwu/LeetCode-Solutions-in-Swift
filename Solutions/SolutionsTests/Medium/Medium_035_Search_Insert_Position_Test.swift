...
class Medium_035_Search_Insert_Position_Test: XCTestCase {
    ...
    func test_001() {
        var input: [AnyObject] = [[1,3,5,6], 5]
        var expected: Int = 2
        asyncHelper(input: input, expected: expected)
    }
    func test_002() {
        var input: [AnyObject] = [[1,3,5,6], 2]
        var expected: Int = 1
        asyncHelper(input: input, expected: expected)
    }
    func test_003() {
        var input: [AnyObject] = [[1,3,5,6], 7]
        var expected: Int = 4
        asyncHelper(input: input, expected: expected)
    }
    func test_004() {
        var input: [AnyObject] = [[1,3,5,6], 0]
        var expected: Int = 0
        asyncHelper(input: input, expected: expected)
    }
    func test_005() {
        var input: [AnyObject] = [[], 999]
        var expected: Int = 0
        asyncHelper(input: input, expected: expected)
    }
    func test_006() {
    ...
    func test_008() {
        var input: [AnyObject] = [[1], 2]
        var expected: Int = 1
        asyncHelper(input: input, expected: expected)
    }
    func test_009() {
        var input: [AnyObject] = [[1,3,5,6], 1]
        var expected: Int = 0
        asyncHelper(input: input, expected: expected)
    }
    func test_010() {
        var input: [AnyObject] = [[1,3,5,6], 6]
        var expected: Int = 3
        asyncHelper(input: input, expected: expected)
    }
    private func asyncHelper(var # input: [AnyObject], expected: Int) {
        var expectation: XCTestExpectation = self.expectationWithDescription(Medium_035_Search_Insert_Position_Test.TimeOutName)
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), { () -> Void in
            var result: Int = Medium_035_Search_Insert_Position.searchInsert(nums: input[0] as! [Int], target: input[1] as! Int)
