...
class Medium_073_Set_Matrix_Zeroes_Test: XCTestCase {
    ...
    private func asyncHelper(input ipt: [[Int]], expected: [[Int]]) {
        var input = ipt
        weak var expectation: XCTestExpectation? = self.expectation(withDescription: Medium_073_Set_Matrix_Zeroes_Test.TimeOutName)
        ...
        waitForExpectations(withTimeout: Medium_073_Set_Matrix_Zeroes_Test.TimeOut) { (error: NSError?) -> Void in
            if error != nil {
