...
class Medium_091_Decode_Ways_Test: XCTestCase {
    ...
    private func asyncHelper(input input: String, expected: Int) {
        weak var expectation: XCTestExpectation? = self.expectationWithDescription(Medium_091_Decode_Ways_Test.TimeOutName)
        ...
        waitForExpectationsWithTimeout(Medium_091_Decode_Ways_Test.TimeOut) { (error: NSError?) -> Void in
            if error != nil {
