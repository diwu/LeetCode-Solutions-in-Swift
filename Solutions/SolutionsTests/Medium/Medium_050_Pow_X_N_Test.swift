...
class Medium_050_Pow_X_N_Test: XCTestCase {
    ...
    private func asyncHelper(input: [Any], expected: Double) {
        weak var expectation: XCTestExpectation? = self.expectation(withDescription: Medium_050_Pow_X_N_Test.TimeOutName)
        ...
        waitForExpectations(withTimeout: Medium_050_Pow_X_N_Test.TimeOut) { (error: NSError?) -> Void in
            if error != nil {
