...
class Medium_005_Longest_Palindromic_Substring_Test: XCTestCase {
    ...
    private func asyncHelper(input: String, expected: String) {
        ...
        DispatchQueue.global(attributes: DispatchQueue.GlobalAttributes.qosDefault).async(execute: { () -> Void in
            ...
            assertHelper(expected == result_objc, problemName: Medium_005_Longest_Palindromic_Substring_Test.ProblemName, input: input, resultValue: result_objc, expectedValue: expected)
            if let unwrapped = expectation {
                unwrapped.fulfill()
            }
        })
        waitForExpectations(timeout: Medium_005_Longest_Palindromic_Substring_Test.TimeOut) { (error: NSError?) -> Void in
            if error != nil {
