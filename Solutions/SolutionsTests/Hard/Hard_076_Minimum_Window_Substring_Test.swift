...
class Hard_076_Minimum_Window_Substring_Test: XCTestCase {
    ...
    private func asyncHelper(input input: [String], expected: String) {
        weak var expectation: XCTestExpectation? = self.expectationWithDescription(Hard_076_Minimum_Window_Substring_Test.TimeOutName)
        ...
        waitForExpectationsWithTimeout(Hard_076_Minimum_Window_Substring_Test.TimeOut) { (error: NSError?) -> Void in
            if error != nil {
